...
then
    ...
    then
        ...
        echo "= generate docs ========================================================="
        make generate-docs > "${TMPDIR:-/tmp}/generate-docs.log" 2>&1 && git diff --quiet site && echo ok || (((exitcode += 3)) && echo ERROR: Please run make generate-docs; tail -n 40 "${TMPDIR:-/tmp}/generate-docs.log"; git --no-pager diff --stat site | head -40)
    else
